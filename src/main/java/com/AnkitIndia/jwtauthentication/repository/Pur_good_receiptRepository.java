...
public interface Pur_good_receiptRepository extends JpaRepository<Pur_good_receipt, Long>{
	...
	@Query(value= "select id,grn_id,grn_no,grn_date,b_unitname,supplier,purchase_typename,referance_type,vehicle_no from pur_good_receipt where modified_type ='INSERTED' and fin_year =:finyear and grn_date =:currDate", nativeQuery=true)
	...
	@Query(value="select SUM(gross_amt) FROM pur_good_receipt_item_details where grnid=:grnid and modified_type='INSERETED'",nativeQuery=true)
