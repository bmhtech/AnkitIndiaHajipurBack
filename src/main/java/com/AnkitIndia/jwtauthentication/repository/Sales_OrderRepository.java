...
//	List<Sales_Order> getSaleOrderList(@Param("currDate") String currDate);
	...
	@Query(value= "SELECT s.id AS id,s.order_id AS order_id,s.order_date AS order_date,s.order_no AS order_no,s.order_type AS order_type,(CASE WHEN s.delivery_date IS NULL THEN 'NA' ELSE s.delivery_date END) AS delivery_date,(SELECT reason FROM reason_master WHERE modified_type ='INSERTED' AND reason_id =s.reason) AS q_status,(SELECT cp_name FROM cust_bussiness_partner WHERE modified_type ='INSERTED' AND cp_id =s.customer) AS customer_name,s.ref_type AS ref_type,(SELECT invtype_name FROM invoice_type WHERE modified_type ='INSERTED' AND invtype_id =s.inv_type) AS inv_type,(CASE WHEN s.inv_type = 'INV00003' THEN s.total_job_amt ELSE (SELECT (SUM(si.amount) + SUM(si.tax_amt) - SUM(si.discount_amt)) AS netamt FROM sales_order_item_dtls si WHERE si.modified_type = 'INSERTED' AND si.order_id = s.order_id) END) AS net_amount,s.tolerancecheckpoint AS tolerancecheckpoint,s.loading_status as loading_status,s.terminate as terminate,s.customer FROM sales_order s WHERE s.modified_type='INSERTED' AND s.order_date=:currdate AND s.fin_year=:finyear", nativeQuery=true)
	List<Map<String, Object>> getSaleOrderFastList(@Param("currdate") String currdate, @Param("finyear") String finyear);
	
	@Query(value= "SELECT s.id AS id,s.order_id AS order_id,s.order_date AS order_date,s.order_no AS order_no,s.order_type AS order_type,(CASE WHEN s.delivery_date IS NULL THEN 'NA' ELSE s.delivery_date END) AS delivery_date,(SELECT reason FROM reason_master WHERE modified_type ='INSERTED' AND reason_id =s.reason) AS q_status,(SELECT cp_name FROM cust_bussiness_partner WHERE modified_type ='INSERTED' AND cp_id =s.customer) AS customer_name,s.ref_type AS ref_type,(SELECT invtype_name FROM invoice_type WHERE modified_type ='INSERTED' AND invtype_id =s.inv_type) AS inv_type,(CASE WHEN s.inv_type = 'INV00003' THEN s.total_job_amt ELSE (SELECT (SUM(si.amount) + SUM(si.tax_amt) - SUM(si.discount_amt)) AS netamt FROM sales_order_item_dtls si WHERE si.modified_type = 'INSERTED' AND si.order_id = s.order_id) END) AS net_amount,s.tolerancecheckpoint AS tolerancecheckpoint,s.loading_status as loading_status,s.terminate as terminate,s.customer FROM sales_order s WHERE s.modified_type='INSERTED' AND :#{#sql}", nativeQuery=true)
	...
	//@Query(value= "SELECT s.order_id,s.order_no FROM sales_order s WHERE s.modified_type='INSERTED' AND s.inv_type!='INV00003' AND s.fin_year=:fin_year AND s.terminate=0", nativeQuery=true)
	//@Query(value= "SELECT s.order_id,s.order_no,CONCAT(s.order_no,'/',(SELECT IFNULL(c.alt_name,'') AS alt_name FROM cust_bussiness_partner c WHERE c.cp_id=s.customer AND c.modified_type='INSERTED'),'/',IFNULL(s.cust_refdocno,'')) AS cust_order_no FROM sales_order s WHERE s.modified_type='INSERTED' AND s.inv_type!='INV00003' AND s.fin_year=:fin_year AND s.terminate=0", nativeQuery=true) //Commented as it show all sales order in grn
	@Query(value= "SELECT s.order_id,s.order_no,CONCAT(s.order_no,'/',(SELECT IFNULL(c.alt_name,'') AS alt_name FROM cust_bussiness_partner c WHERE c.cp_id=s.customer AND c.modified_type='INSERTED'),'/',IFNULL(s.cust_refdocno,'')) AS cust_order_no FROM sales_order s WHERE s.modified_type='INSERTED' AND s.inv_type!='INV00003' AND s.fin_year=:fin_year AND s.terminate=0 AND (SELECT c.group_type FROM cust_bussiness_partner c WHERE c.cp_id=s.customer AND c.modified_type='INSERTED')!='CG00019'", nativeQuery=true)
