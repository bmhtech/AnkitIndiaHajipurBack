...
//	List<Sales_Order> getSaleOrderList(@Param("currDate") String currDate);
	...
	@Query(value= "SELECT s.id AS id,s.order_id AS order_id,s.order_date AS order_date,s.order_no AS order_no,s.order_type AS order_type,(CASE WHEN s.delivery_date IS NULL THEN 'NA' ELSE s.delivery_date END) AS delivery_date,(SELECT reason FROM reason_master WHERE modified_type ='INSERTED' AND reason_id =s.reason) AS q_status,(SELECT cp_name FROM cust_bussiness_partner WHERE modified_type ='INSERTED' AND cp_id =s.customer) AS customer_name,s.ref_type AS ref_type,(SELECT invtype_name FROM invoice_type WHERE modified_type ='INSERTED' AND invtype_id =s.inv_type) AS inv_type,(CASE WHEN s.inv_type = 'INV00003' THEN s.total_job_amt ELSE (SELECT (SUM(si.amount) + SUM(si.tax_amt) - SUM(si.discount_amt)) AS netamt FROM sales_order_item_dtls si WHERE si.modified_type = 'INSERTED' AND si.order_id = s.order_id) END) AS net_amount,s.tolerancecheckpoint AS tolerancecheckpoint,s.loading_status as loading_status,s.terminate as terminate,s.referance_id as referance_id FROM sales_order s WHERE s.modified_type='INSERTED' AND s.order_date=:currdate AND s.fin_year=:finyear", nativeQuery=true)
