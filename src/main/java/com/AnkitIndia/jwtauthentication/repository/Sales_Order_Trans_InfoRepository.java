...
public interface Sales_Order_Trans_InfoRepository extends JpaRepository<Sales_Order_Trans_Info, Long> {
	...
	@Query( value="select s.* from sales_order_trans_info s where s.modified_type = 'INSERTED' and s.order_id =:order_id ",nativeQuery = true)
	Map<String,Object> getSalesOrdTransInfoFast(@Param("order_id") String order_id);
