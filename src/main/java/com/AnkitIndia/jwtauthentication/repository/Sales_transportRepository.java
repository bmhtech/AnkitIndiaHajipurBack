...
public interface Sales_transportRepository extends JpaRepository<Sales_transport, Long>{
	...
	@Query(value = "SELECT l.ledgername FROM sales_transport s JOIN delivery_challan_chgs_dyn d ON d.challan_no=s.challanno AND d.modified_type='INSERTED' JOIN ledgermaster l ON l.ledgerid=d.transportation_acc AND l.modified_type='INSERTED' WHERE s.modified_type='INSERTED' AND s.id=:id", nativeQuery=true)
	String getSalesTransportLedger(@Param("id") String id);
